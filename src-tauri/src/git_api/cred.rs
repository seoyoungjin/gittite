...
pub fn extract_username_password(repo_path: &RepoPath) -> Result<BasicAuthCredential> {
    let repo = repo_open(repo_path)?;
    let url = repo
    ...
    Ok(match helper.execute() {
        Some((username, password)) => BasicAuthCredential::new(Some(username), Some(password)),
...
mod tests {
    use crate::git_api::{
        cred::{
            extract_cred_from_url, extract_username_password, need_username_password,
            BasicAuthCredential,
        },
    ...
    fn test_credential_complete() {
        assert_eq!(
            BasicAuthCredential::new(Some("username".to_owned()), Some("password".to_owned()))
                .is_complete(),
            true
        );
    }
    ...
    fn test_credential_not_complete() {
        assert_eq!(
            BasicAuthCredential::new(None, Some("password".to_owned())).is_complete(),
            false
        );
        assert_eq!(
            BasicAuthCredential::new(Some("username".to_owned()), None).is_complete(),
            false
        );
        assert_eq!(BasicAuthCredential::new(None, None).is_complete(), false);
    }
    ...
    fn test_extract_username_password_from_url() {
        assert_eq!(
            ...
            BasicAuthCredential::new(Some("user".to_owned()), Some("pwd".to_owned()))
        );
    }
    ...
    fn test_dont_need_username_password_if_pushurl_ssh() {
        ...
        repo.remote(DEFAULT_REMOTE_NAME, "http://user@github.com")
            .unwrap();
        repo.remote_set_pushurl(DEFAULT_REMOTE_NAME, Some("git@github.com:user/repo"))
            .unwrap();
    ...
    fn test_extract_username_password_from_repo() {
        ...
        repo.remote(DEFAULT_REMOTE_NAME, "http://user:pass@github.com")
            .unwrap();

        assert_eq!(
            extract_username_password(repo_path).unwrap(),
            BasicAuthCredential::new(Some("user".to_owned()), Some("pass".to_owned()))
        );
    }
