...
fn write_diff_line(
    ...
    write!(buf, "{}", str::from_utf8(line.content()).unwrap());
    true
}

pub fn diff<I>(repo_path: &RepoPath, args: I) -> Result<String, Error>
where
    I: IntoIterator,
    I::Item: Into<OsString> + Clone
{
    ...
    if args.flag_blobs {
        ...
        repo.diff_blobs(
            ...
            None,
            None,
            Some(&mut |d, h, l| write_diff_line(&mut buf, d, h, l)),
        )?;
    ...
    if args.flag_patch || !stats {
        diff.print(args.diff_format(), |d, h, l| write_diff_line(&mut buf, d, h, l))?;
    }
