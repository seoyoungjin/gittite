...
pub fn add_to_ignore(
    ...
    if Path::new(path_to_ignore).file_name() == Path::new(GITIGNORE).file_name() {
        return Err(Error::Generic(String::from("cannot ignore gitignore")));
    }

    let ignore_file = work_dir(&repo)?.join(GITIGNORE);

    let optional_newline = ignore_file.exists() && !file_ends_with_newline(&ignore_file)?;
...
mod tests {
    use super::*;
    use crate::git_api::{tests::repo_init, utils::repo_write_file};
    use io::BufRead;
    ...
    fn test_append() -> Result<()> {
        ...
        File::create(&root.join(file_path))?.write_all(b"test")?;
        File::create(&root.join(ignore_file_path))?.write_all(b"foo\n")?;

        add_to_ignore(repo_path, file_path.to_str().unwrap())?;

        let mut lines = read_lines(&root.join(ignore_file_path)).unwrap();
