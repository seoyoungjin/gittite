...
use crate::git_api::{cred::BasicAuthCredential, error::Result, progress::RemoteProgress};
...
impl Callbacks {
    ...
    pub fn new(
        ...
        Self {
            ...
            basic_credential,
            stats,
            first_call_to_credentials: Arc::new(AtomicBool::new(true)),
        }
    }
    ...
    fn push_update_reference(
        ...
        if let Ok(mut stats) = self.stats.lock() {
            stats.push_rejected_msg = msg.map(|msg| (reference.to_string(), msg.to_string()));
        }
    }
    ...
    fn transfer_progress(
        ...
        log::debug!("transfer: {}/{}", p.received_objects(), p.total_objects());
        self.sender.clone().map(|sender| {
            sender.send(
    ...
    fn credentials(
        ...
        match &self.basic_credential {
            _ if allowed_types.is_ssh_key() => username_from_url.map_or_else(
                || Err(GitError::from_str(" Couldn't extract username from url.")),
                Cred::ssh_key_from_agent,
            ),
            Some(BasicAuthCredential {
                username: Some(user),
                password: Some(pwd),
            }) if allowed_types.is_user_pass_plaintext() => Cred::userpass_plaintext(user, pwd),
            Some(BasicAuthCredential {
                username: Some(user),
                password: _,
